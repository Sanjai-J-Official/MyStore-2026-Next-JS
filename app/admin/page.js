...
export default async function AdminDashboard({ searchParams }) {
  ...
  if (searchParams.key !== 'mysecretadmin123') {
    return (
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.actions}>
          <Link href="/admin/products?key=mysecretadmin123" className={styles.btnPrimary}>
            Manage Products
          </Link>
          <Link href="/admin/products/new?key=mysecretadmin123" className={styles.btnPrimary} style={{ background: 'var(--text-primary)' }}>
            + Add Product
          </Link>
        </div>
      </div>
      ...
      <div className={styles.statsGrid}>
        ...
        <div className={styles.statCard}>
          ...
          <div className={styles.statValue} style={{ color: lowStockItems > 0 ? 'var(--error)' : 'var(--success)' }}>
            {lowStockItems}
          </div>
        </div>
      </div>
            ...
            <tbody>
              {orders.slice(0, 10).map((order) => (
                <tr key={order._id}>
                  ...
                  <td>
                    <span className={`${styles.statusBadge} ${styles[`status_${order.paymentStatus}`]}`}>
                      {order.paymentStatus}
                    </span>
                  </td>
                  <td>
